        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                @if(Auth::user()->canEditProject($project->id))
                    <a href="{{ route('admin.projects.admins.create', $project) }}" class="btn btn-primary">
                                            ...
                                            <div class="dropdown-menu">
                                                @if(Auth::user()->canEditProject($project->id))
                                                    @if($admin->id != $project->pic_user_id)
                                                        <a href="{{ route('admin.projects.admins.edit', [$project, $admin]) }}" 
                                                           class="dropdown-item">
                                                        ...
                                                        @if($admin->id != Auth::id())
                                                            <form action="{{ route('admin.projects.admins.toggle-status', [$project, $admin]) }}" 
                                                            ...
                                                            <button type="button" class="dropdown-item text-danger delete-btn"
                                                                    ...
                                                                    data-url="{{ route('admin.projects.admins.destroy', [$project, $admin]) }}">
                                                    ...
                                                    @else
                                                        <span class="dropdown-item text-muted">
                                                            ...
                                                            Admin PIC - Edit melalui halaman project
                                                        </span>
                                                    @endif
                                                @else
                                                    <span class="dropdown-item text-muted">
                                                        <i class="ti ti-lock me-2"></i>
                                                        Hanya PIC yang dapat mengelola admin
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (resetPasswordModal && resetPasswordForm) {
        resetPasswordModal.addEventListener('show.bs.modal', function(event) {
            ...
            // Update form action
            resetPasswordForm.action = `{{ route('admin.projects.admins.index', $project) }}/${adminId}/reset-password`;
