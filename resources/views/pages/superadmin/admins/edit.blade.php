                ...
                <div class="card-body">
                    @if($admin->adminProjects->count() > 0)
                        <div class="list-group list-group-flush">
                            @foreach($admin->adminProjects as $projectAdmin)
                            <div class="list-group-item d-flex align-items-center px-0">
                                @if($projectAdmin->project->logo)
                                    <img src="{{ $projectAdmin->project->logo_url }}" alt="{{ $projectAdmin->project->name }}" 
                                         class="avatar avatar-sm me-2">
                                @else
                                    <div class="avatar avatar-sm bg-primary-lt me-2">
                                        {{ substr($projectAdmin->project->name, 0, 1) }}
                                    </div>
                                @endif
                                <div class="flex-fill">
                                    <div class="fw-bold">{{ $projectAdmin->project->name }}</div>
                                    <div class="text-secondary small">
                                        <span class="badge bg-{{ $projectAdmin->project->is_active ? 'success' : 'secondary' }}">
                                            {{ $projectAdmin->project->is_active ? 'Aktif' : 'Tidak Aktif' }}
                                        </span>
                                    </div>
                                </div>
                            </div>
